...
| 表 | 关键字段 | 写入流程 |
|----|----------|----------|
| `platform_accounts` | id / name / platform / login_status / enabled / cdp_port | F1 / F2 / F5 |
| `nurture_tasks` | id / account_id / status / progress / result_json / error | F2 / F3 / F5 |
| `nurture_schedules` | id / name / cron / account_ids / action_set_id / enabled | F3 |
| `favorite_snapshots` | id / account_id / captured_at / item_count / items_json | F4 |
| `system_settings` | key / value | F2 / F6 |
| `notifications` | id / event_type / level / payload / read_at | F6 |
| `users` | id / username / password_hash / role | F7 |
...
> 涉及表：`platform_accounts`、`audit_logs`、`risk_events`（如有）。
...
sequenceDiagram
    ...
    Op->>FE: 3. 提交表单
    FE->>API: 4. POST /api/v1/platform-accounts<br/>{name, platform}
    API->>DB: 5. INSERT platform_accounts<br/>(status=pending_login, enabled=false, cdp_port=null)
    Note over DB: platform_accounts:<br/>id=42, name="xhs-小A",<br/>platform=xhs, login_status=pending_login
    API->>Pool: 6. acquire_free_port()
    Pool-->>API: 7. cdp_port=9223
    API->>DB: 8. UPDATE platform_accounts<br/>SET cdp_port=9223
    API-->>FE: 9. 201 Created {id: 42, cdp_port: 9223}
    FE->>Op: 10. 弹二维码 ElDialog<br/>提示「请用平台 App 扫码」

    Op->>Plat: 11. 平台 App 扫码登录
    Op->>FE: 12. 点击「我已扫码」
    FE->>API: 13. POST /api/v1/platform-accounts/42/check-login
    ...
    alt 已登录
        ...
        API->>DB: 20. UPDATE platform_accounts<br/>SET login_status=valid,<br/>platform_user_id="abc123"
        ...
        API->>DB: 20'. UPDATE platform_accounts<br/>SET login_status=cookie_invalid
    ...
    Op->>FE: 24. 点击「启用此账号」
    FE->>API: 25. PUT /api/v1/platform-accounts/42<br/>{enabled: true}
    API->>DB: 26. UPDATE platform_accounts<br/>SET enabled=true
...
```sql
-- 初始创建
INSERT INTO platform_accounts (name, platform, login_status, enabled, cdp_port, ...)
VALUES ('xhs-小A', 'xhs', 'pending_login', false, NULL, ...);
-- cdp_port: NULL → 9223
-- login_status: pending_login → valid | cookie_invalid | banned
-- enabled: false → true
-- platform_user_id: NULL → 'abc123'
```
...
> 涉及表：`nurture_tasks`、`platform_accounts`、`system_settings`、`audit_logs`。
...
sequenceDiagram
    ...
    Op->>FE: 3. 提交
    FE->>API: 4. POST /api/v1/platform-accounts/42/nurture<br/>{action_set_id: 7, duration_minutes: 30}
    API->>DB: 5. SELECT platform_accounts WHERE id=42
    API->>DB: 6. SELECT system_settings WHERE key='nurture_global_enabled'
    alt 守卫不通过 (见 2.2)
        API-->>FE: 7'. 4xx {error: "..."}
        FE->>Op: 8'. ElMessage.error
    else 全部通过
        API->>DB: 9. INSERT nurture_tasks<br/>(account_id=42, status=pending, ...)
        Note over DB: nurture_tasks:<br/>id=2024, account_id=42,<br/>status=pending, progress=0
        API->>Q: 10. nurture.run.delay(<br/>account_id=42, actions=[...],<br/>duration=30, task_id=2024)
        Q-->>API: 11. enqueued
    ...
    loop Worker 消费
        Q->>W: 14. pop task
        W->>DB: 15. UPDATE nurture_tasks<br/>SET status=running, started_at=now
        W->>DB: 16. UPDATE platform_accounts<br/>SET last_nurture_at=now
        ...
        loop 串行执行每个 action
            ...
            W->>DB: 25. UPDATE nurture_tasks<br/>SET progress=progress+10%<br/>, result_json=json_append(...)
            Note over DB: nurture_tasks.progress:<br/>0 → 10 → 30 → 60 → 100
        ...
        Plat-->>Br: 29. 收藏夹列表
        Br-->>W: 30. list[FavoriteItem]
        W->>DB: 31. INSERT favorite_snapshots<br/>(account_id=42, items_json=..., item_count=N)
        W->>DB: 32. UPDATE nurture_tasks<br/>SET status=success, progress=100,<br/>finished_at=now
        W->>DB: 33. INSERT audit_logs<br/>(action=nurture, result=success)
        W->>Br: 34. release context to ChromePool
    end

    loop FE 5s 轮询
        FE->>API: 35. GET /api/v1/nurture-tasks/2024
        API->>DB: 36. SELECT * FROM nurture_tasks WHERE id=2024
...
| 账号 `enabled == true` | 400 `account_disabled` | `platform_accounts` |
| 账号 `login_status == valid` | 400 `login_invalid` | `platform_accounts` |
| 当前时间不在 `SILENT_HOURS` (0-6) | 400 `silent_hours` | `policy.SILENT_HOURS` |
| `nurture_tasks.today_used(account_id) < daily_quota_seconds` | 429 `quota_exceeded` | `nurture_tasks` 聚合 |

### 2.3 失败分支

| 失败点 | 表现 | 处置 |
|--------|------|------|
| 任一守卫不通过 | 启动阶段 4xx | 弹错误 toast，**不创建** `nurture_tasks` |
| 第 19 步 check_login 失败 | `login_status=cookie_invalid` | 任务标记 `failed`、不重试；前端 toast 提示"请重新扫码" |
...
| Worker 进程崩 | `nurture_tasks.status` 长期 `running` | admin 可手动标记 `failed`（`/nurture/history`） |
...
> 涉及表：`nurture_schedules`、`nurture_tasks`、`platform_accounts`、`system_settings`。
...
sequenceDiagram
    ...
    FE->>API: 4. POST /api/v1/nurture-schedules
    API->>DB: 5. INSERT nurture_schedules<br/>(cron, account_ids=[42,43,44],<br/>action_set_id=7, enabled=true)
    ...
    Beat->>DB: 9. SELECT * FROM nurture_schedules<br/>WHERE enabled=true AND next_run_at<=now
    DB-->>Beat: 10. [schedule#1]
    loop 每个 schedule 触发的每个 account_id
        Beat->>Q: 11. nurture.run.delay(<br/>account_id, actions, duration,<br/>triggered_by_schedule_id=1)
        Q-->>Beat: 12. enqueued
        Beat->>DB: 13. UPDATE nurture_schedules<br/>SET last_run_at=now,<br/>next_run_at=compute_next(cron)
    end

    Q->>W: 14. pop task
    W->>W: 15. 进入"流程 2"的所有步骤<br/>(check_login → actions → fetch_favorites)
    W->>DB: 16. UPDATE nurture_tasks<br/>SET triggered_by_schedule_id=1

    alt 连续 N 次失败
        W->>DB: 17. SELECT count failed for (account_id, schedule_id)
        Note over DB: 累计失败次数 >= 3
        W->>DB: 18. UPDATE platform_accounts<br/>SET enabled=false, reason='auto_disabled:schedule'
...
| 字段 | 来源表 | 作用 |
|------|--------|------|
| `nurture_tasks.triggered_by_schedule_id` | FK → `nurture_schedules.id` | 区分手动触发 vs 定时触发 |
| `nurture_schedules.last_run_at` | 每次 beat 触发更新 | 监控 beat 是否正常 |
| `nurture_schedules.next_run_at` | 由 cron 推算 | UI 显示"下次触发" |
...
> 涉及表：`favorite_snapshots`、`platform_accounts`、`notifications`。
...
sequenceDiagram
    ...
    W->>DB: A5. INSERT favorite_snapshots<br/>(account_id=42, captured_at=now,<br/>item_count=N, items_json=JSON)

    Note over Op,Plat: 触发方式 B: 操作员手动触发
    Op->>FE: B1. /nurture/favorites 点「立即抓取」
    FE->>API: B2. POST /api/v1/platform-accounts/42/favorites/refresh
    API->>Q: B3. fetch_favorites.delay(account_id=42)
    ...
    W->>DB: B14. INSERT favorite_snapshots
...
| 对比项 | 字段 | 算法 |
|--------|------|------|
| 新增收藏 | `items_json[N] - items_json[N-1]` | set diff on `note_id` |
| 删除收藏 | `items_json[N-1] - items_json[N]` | set diff on `note_id` |
| 收藏总数变化 | `item_count[N] - item_count[N-1]` | int diff |
...
> 涉及表：`nurture_tasks`、`platform_accounts`、`notifications`、`audit_logs`。
...
sequenceDiagram
    ...
    W->>DB: 5. UPDATE nurture_tasks<br/>SET status=failed_retrying,<br/>retry_count=1, error='rate_limit'
    ...
    alt 成功
        W->>DB: 12. UPDATE nurture_tasks<br/>SET status=success, retry_count=2
    else 再次失败
        W->>DB: 13. UPDATE nurture_tasks<br/>SET retry_count=2, error='...'
        W->>Q: 14. self.retry(countdown=2^2 * 60 = 240s)
    end

    Note over W,Q: 累计 retry_count >= MAX_RETRIES (默认 3)
    W->>DB: 15. UPDATE nurture_tasks<br/>SET status=failed (终态)
    W->>DB: 16. SELECT count(*)<br/>FROM nurture_tasks<br/>WHERE account_id=42 AND status=failed<br/>AND created_at > now - 24h
    alt 24h 内失败 >= 3 次
        W->>DB: 17. UPDATE platform_accounts<br/>SET enabled=false,<br/>reason='auto_disabled:too_many_failures'
...
下次 beat 触发时守卫拒绝（quota_exceeded / silent_hours）
  ↓
可选：PUT /api/v1/platform-accounts/{id} enabled=false 全停
  ↓
...
celery worker 进程 kill -9
  ↓
nurture_tasks.status='running' 卡住
  ↓
60 min 后 admin 在 /nurture/running 发现无变化
  ↓
PUT /api/v1/nurture-tasks/{id} status=failed (手动标记)
  ↓
F6 通知: worker_stuck
  ↓
...
adapter.check_login 返回 captcha_required
  ↓
platform_accounts.login_status='cookie_invalid'
  ↓
...
| 流程 | 图编号 | 关键表 | 失败重试策略 |
|------|--------|--------|--------------|
| F1 账号接入 | §1.1 | platform_accounts, audit_logs | 扫码循环 ≤ 3 次 |
| F2 单次养号 | §2.1 | nurture_tasks, system_settings | `max_retries=0` 默认 |
| F3 定时养号 | §3.1 | nurture_schedules, nurture_tasks | 失败 N 次自动禁号 |
| F4 收藏夹抓取 | §4.1 | favorite_snapshots | 不重试（手动） |
| F5 失败重试 | §5.1 | nurture_tasks, platform_accounts | 指数退避 60/120/240/480s |
| F6 通知触发 | §6.1 | notifications, system_settings | 渠道 3 次重试 |
| F7 操作员登录 | §7.1 | users, permissions, audit_logs | 5 次失败锁 15 min |
